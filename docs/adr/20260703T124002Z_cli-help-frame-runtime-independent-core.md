...
- `dispatchCli` only handles `--help` / `-h`, and falls back to an unknown-command/unknown-option error otherwise; the entrypoints built in #81/#82 must still decide how to surface that error (e.g. writing `stderr` and calling `process.exit` with its `exitCode`), and each subcommand issue (#88-#91) must replace the unknown-command fallback with real handling and its own exit code for its own command.
