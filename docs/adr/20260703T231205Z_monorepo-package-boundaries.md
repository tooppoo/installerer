...
`packages/cli/package.json` is a static, reviewable file containing the published CLI metadata (`name`, `bin`, `files`, `engines`, repository metadata). The normal build/test flow never generates a CLI `package.json` from scratch. `build:npm` (now `packages/cli/scripts/build-npm.ts`) assembles the publish directory `packages/cli/dist/npm/` by bundling `src/node/main.ts` and projecting the static manifest, stripping only workspace-only fields (`$schema`, `scripts`, `devDependencies` — the latter holds the `workspace:*` reference to `@installerer/core`, which is bundled into the bin artifact and never installed by consumers). The one non-static field is `version`: installerer's canonical version is the repository root `package.json` `version`, so the published npm manifest receives that value.

This restores the property the generated-manifest approach lost: almost all manifest fields that ship are diffable in review and changes to them appear in PRs. Version remains deliberately centralized at the repository root so the CLI `--version`, npm publish manifest, standalone release archives, and Web UI all report the same installerer version. `packages/cli/src/version.ts` statically imports the repository root `package.json`; the boundary check allows this specific root-manifest import while continuing to reject arbitrary package escapes.
...
- [docs/adr/20260703T134302Z_npm-node-cli-package.md](./20260703T134302Z_npm-node-cli-package.md) is amended by this ADR (generated manifest → mostly static manifest; `dist-cli/npm/` → `packages/cli/dist/npm/`); its CI-verification, entrypoint-layering, and source-map decisions stand. [docs/adr/20260703T133536Z_cli-version-source.md](./20260703T133536Z_cli-version-source.md)'s root-version source remains the canonical installerer version source.
