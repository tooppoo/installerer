...
- If it matches a known command: `--help`/`-h` and `--version`/`-v` are still honored ahead of it (checked with a plain `Array.includes`, not a full parse) so `installerer validate --help` reports help instead of running `validate`. Otherwise, the remaining `rest` of argv is handed to that command's `run` untouched. Each command module now owns parsing its own arguments and choosing its own exit code for its own argument errors: `validate` declares only `--config` (via its own `parseArgs` call, `multiple: true` so a repeated `--config` becomes detectable duplication rather than silently overwriting) and maps any argument problem — an unrecognized option, an unexpected positional, a missing or duplicated `--config` — to `invalidValidateArguments` (8). `init` has no options of its own, so it rejects any leftover `args` itself and reuses the existing `unknownOption` (2), since it has no command-specific cause to introduce.
...
- Each future command module must now implement its own `parseArgs` call and its own argument-error mapping; there is no shared "declare an option, get validation for free" mechanism. This is an accepted, deliberate trade-off given the alternative's scaling problem above.
