 ...
 * constructions the generator is known to emit (6 for a {version} archive
 * template — checksum-index fetch, tag-specific refetch, and pinned install
 * each contribute their own URLs — 5 without {version} — including the one
 * reference in the effective-config metadata comment's generator.sourceUrl
 * field) and that no GitHub reference survives the rewrite, so a test run
 * can never silently fall through to the real network.
 */
