...
REPO='rellog'
BINARY_NAME='rellog'
BINARY_PATH_IN_ARCHIVE='bin/rellog'
...
usage() {
  ...
  printf '%s\n' '  curl -fsSL https://raw.githubusercontent.com/tooppoo/rellog/refs/heads/main/install.sh | sh -s -- --version v0.1.2'
  printf '%s\n' '  curl -fsSL https://raw.githubusercontent.com/tooppoo/rellog/refs/heads/main/install.sh | sh -s -- --install-dir "$HOME/bin"'
  printf '%s\n' ''
  printf '%s\n' 'review-first alternative:'
  printf '%s\n' '  curl -fsSLO https://raw.githubusercontent.com/tooppoo/rellog/refs/heads/main/install.sh'
  printf '%s\n' '  sh ./install.sh --help'
  printf '%s\n' '  sh ./install.sh'
}
...
resolve_asset_arch_label() {
  ...
  case "$canonical_os/$canonical_arch" in
    linux/x86_64) asset_arch_label='x64' ;;
    linux/aarch64) asset_arch_label='arm64-v8a' ;;
