 ...
 * constructions the generator is known to emit (5 with a version file
 * resolver, 4 without) and that no GitHub reference survives the rewrite, so
 * a test run can never silently fall through to the real network.
 */
export function rewriteBaseUrlForTest(script: string, baseUrl: string): string {
  ...
  expect([4, 5]).toContain(occurrences);
